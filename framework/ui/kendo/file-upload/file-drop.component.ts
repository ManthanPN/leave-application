...
import { FieldTypeConfig, FieldWrapper } from '../../../core/src/lib/core';
...
export class FileDropComponent extends FieldWrapper<FieldTypeConfig> implements OnDestroy {
  ...
  @Output()
  public onFileDrop: EventEmitter<FileDropEntry[]> = new EventEmitter();
  ...
  // custom templates
  @ContentChild(FileDropContentTemplateDirective, { read: TemplateRef }) contentTemplate?: TemplateRef<any>;
  ...
  private getFakeDropEntry(file: File): FileDropEntry {
  ...
  private checkFile(item: DataTransferItem | File): void {
    ...
    if ("webkitGetAsEntry" in item) {
      ...
      if (entry) {
        if (entry.isFile) {
          const toUpload: FileDropEntry = new FileDropEntry(entry.name, entry);
  ...
  private traverseFileTree(item: FileSystemEntry, path: string): void {
    if (item.isFile) {
      const toUpload: FileDropEntry = new FileDropEntry(path, item);
  ...
  private addToQueue(item: FileDropEntry): void {
    ...
    this.field.props['onFileUploadLoad'](this);
